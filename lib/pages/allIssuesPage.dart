...
class AllIssuesPageState extends State<AllIssuesPage> {
  ...
  Future<List<GetAllIssues>> fetchAllIssues() async {
    ...
    if (response.statusCode == 200) {
      ...
      //customers list
      uniqueCustomers = allIssues
          .map((issue) => issue.customer)
          .toSet()
          .where((customer) => customer != null && customer.isNotEmpty)
          .toList();

      //countries list
      uniqueCountries = allIssues
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: (userRole == 'Global Manager' ||
                                ...
                                builder: (context, snapshot) {
                                  if (snapshot.connectionState ==
                                    ...
                                    final int criticalCount =
                                        graphData.counts.criticalCount ?? 0;
                                    final int amberCount =
                                        graphData.counts.amberCount ?? 0;
                                    final int trackingCount =
                                        graphData.counts.trackingCount ?? 0;
                                    final int totalCount =
                                        graphData.counts.totalCount ?? 0;
                                    ...
                                    return Center(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text('Looks like the server is under maintenance.',
                                          style: GoogleFonts.poppins(
                                            color: Colors.white30,
                                            fontSize: 20,
                                            fontWeight: FontWeight.w700
