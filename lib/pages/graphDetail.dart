...
class _GraphDetailState extends State<GraphDetail> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    futureIssues = fetchAllIssues(); // Fetch issues when the widget initializes
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            Tab(child: Text('Critical', style: TextStyle(color: Colors.red),),),
            Tab(child: Text('Amber', style: TextStyle(color: Colors.orange),),),
            Tab(child: Text('Tracking', style: TextStyle(color: Colors.green),),),
          ],
        ),
      ),
      body: Container(
